...
(function () {
  ...
  async function init() {
    try {
      ...
      // Auto-analyze the default symbol so dashboard shows data
      if (state.candles.length >= 50) {
        try {
          var autoSignal = window.TradingSignalEngine.generateSignal(
            state.selectedSymbol,
            state.selectedTimeframe,
            state.candles,
            state.config
          );
          state.currentSignal = autoSignal;
          // Save to storage
          window.TradingStorage.saveSignal(autoSignal).catch(function(){});
        } catch (autoErr) {
  ...
  function renderChartWidget() {
    ...
    // Set explicit dimensions before creating chart
    ...
    if (typeof LightweightCharts === 'undefined') {
      container.innerHTML = '<div class="empty-state"><p>Chart library not loaded. Please refresh.</p></div>';
      return;
    }
  ...
  function renderChart() {
    ...
    var html =
      '<div class="chart-page" style="display:flex;flex-direction:column;height:calc(100vh - 56px);">' +
        // Chart toolbar
        ...
        '</div>' +
        // Main chart
        '<div id="chart-container" style="flex:1;width:100%;min-height:450px;"></div>' +
        // MACD sub-chart
        '<div id="macd-chart-container" style="width:100%;height:150px;"></div>' +
        // Analysis panel
    ...
    // Render chart widget after DOM is ready
    if (state.candles.length > 1) {
      requestAnimationFrame(function() {
        setTimeout(function() { renderChartWidget(); }, 50);
      });
    }
